...
lb <- round(slope - crit_t*slop_se,3)
ub <- round(slope + crit_t*slop_se,3)
...
wrongg1 <- round(lala$coefficients[1,1] - crit_t*lala$coefficients[1,2],3)
wrongg2 <- round(lala$coefficients[1,1] + crit_t*lala$coefficients[1,2],3)
...
choices[4] <- paste0("(", wrongg1 ,", ", wrongg2, ")")
...
summary(model)
```
What is the 95% confidence interval for the slope? Round your answer to 3 decimal places. You may assume the critical value $t^*$ is `r crit_t`
...
$$ \beta_1 \pm t^* \times SE_{\beta_1}$$
From the output, $\beta_1 = $ `r slope`, and $SE_\{\beta_1} = $ `r slop_se`
